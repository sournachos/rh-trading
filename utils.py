...
def get_closest_strike_price(ticker, call_or_put):
    ...
    if r.options.find_options_by_strike(
        ticker, half_dollar_increment, call_or_put, info="expiration_date"
    ):
        print(f"Closest strike price: {half_dollar_increment}")
        return half_dollar_increment
    if r.options.find_options_by_strike(
        ticker, (half_dollar_increment + 0.5), call_or_put, info="expiration_date"
    ):
        print(f"Closest strike price: {half_dollar_increment + .5}")
        return half_dollar_increment + 0.5


def get_nearest_out_of_the_money_option_contract_details(
    ticker, call_or_put, exp_date: Optional[str] = None
):
    """
    ...
    try:
        ...
        print(
            f"Something went wrong retrieving option contract details for {ticker}: {e}"
        )


def buy_option_limit_order(
    ticker, call_or_put, strike_price, exp_date, quantity, option_price
):
    try:
